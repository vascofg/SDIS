...
public class Message {
	...
	public Message(byte[] messageBytes, int length) throws Exception { // quando
																		// recebe
		int i;
			...
			if (messageBytes[i] == '\r' && messageBytes[i + 1] == '\n') {
				if (messageBytes[i + 2] == '\r' && messageBytes[i + 3] == '\n') {
					...
					if (i != length) // mensagem com body
					{
						...
						System.arraycopy(messageBytes, i, this.chunkData, 0,
								length - i);
						this.chunk = new Chunk(header.fileId, header.ChunkNo,
								header.RepDeg, length - i);
					}
					return;
				}
			}
